...
const ContentProducts = ({ categorie }) => {
	...
	useEffect(() => {
		if (dataProducts) {
			if (textSearch) {
				...
				const productsFilterCat = allProducts.filter((product) =>
					product.categories.some(
						(subCat) =>
							subCat.name.toLowerCase() === filterCategorie.toLowerCase()
					)
				);
				...
				const productsFilterCateg = allProducts?.filter((product) =>
					product.categories.some((subCat) =>
						subCat.name.toLowerCase().includes(categorie.toLowerCase())
					)
				);
	...
	const categories = allProducts?.map((prod) =>
		prod.categories.map((subCat) => subCat.name).join()
	);
	const listCategorie = new Set(categories.flat());

	const listProductsByCat = (cat) => {
		if (cat !== "all") {
			const productByCat = allProducts.filter((product) =>
				product.categories.some(
					(subCat) => subCat.name.toLowerCase() === cat.toLowerCase()
				)
			);
			...
			<div
				...
				{allProducts.length > 0 && (
					<SearchAside
						...
						recibirValuePrecio={recibirFiltroPrecio}
				...
				{widthWindow < 875 && (
					<div className="flex items-center gap-2 pt-[20px] px-[20px] md:px-[80px]">
						<label
							className="flex gap-1 w-[auto] cursor-pointer text-sm  sm:text-[16px] text-pink-500  hover:text-gray-600"
							onClick={() => setIsOpenNavCategorie(!isOpenNavCategorie)}>
							FILTRAR <FilterOutlined />{" "}
						</label>
						{!isOpenNavCategorie ? (
							<span className="capitalize text-pink-400 text-sm  sm:text-[16px]">
								&raquo; {nameCategorie}{" "}
							</span>
						) : (
							""
